...
<body>
   ...
   List<Community> list = communityRecommendDaoImpl.selectList();
   ...
   int count = 0;
   count = communityRecommendDaoImpl.getCount(); // 데이터베이스에 저장된 총 갯수
   ...
   if (count > 0) {
      ...
      list2 = communityRecommendDaoImpl.getList(startRow, endRow);
   }
   %>
   ...
   <div style="width: 67%; margin: auto">
      ...
      <table class="comm_table">
         ...
         if (count > 0) { // 데이터베이스에 데이터가 있으면
            ...
            for (int i = 0; i < list2.size(); i++) {
               ...
               String level = member3.printLevelImage();
         %>
         <tr>
            <td style="text-align:left;padding-left:50px;width:58%"><a href="comm_recommend_view.jsp?num=<%=num%>"><%=board.getSubject()%></a></td>
            <th class="comm_table_space"></th>
            <td style="width:19%;"><img src="image/<%=level%>" style="width:25px; height:25px"/> <%=board.getEmail()%>(<%=name%>)</td>
            <th class="comm_table_space"></th>
      ...
      <button style="cursor:pointer" class="comm_table_new" type="button"
         onclick="location.href='comm_recommend_write.jsp'">글작성</button>
         <%} %>
	...
	<div style="width:67%;margin:auto;text-align:center">
               ...
               %><button class="comm_pagebutton" type="button" onclick="location.href='comm_recommend.jsp?pageNum=<%=startPage - 10%>'">이전</button>
                <%
 }

 for (int i = startPage; i <= endPage; i++) { // 페이지 블록 번호
 if (i == currentPage) { // 현재 페이지에는 링크를 설정하지 않음
 %>  <button class="comm_pagebutton_selected" type="button" onclick="location.href='comm_recommend.jsp?pageNum=<%=i%>'"><%=i %></button> <%
 } else { // 현재 페이지가 아닌 경우 링크 설정
 %> <%-- <a class ="comm_button" href="comm_free.jsp?pageNum=<%=i%>">[<%=i%>] --%>
 <button class="comm_pagebutton" type="button" onclick="location.href='comm_recommend.jsp?pageNum=<%=i%>'"><%=i %></button>
            <!-- </a> --> <%
 }
 } // for end

 if (endPage < pageCount) { // 현재 블록의 마지막 페이지보다 페이지 전체 블록수가 클경우 다음 링크 생성
 %><button class="comm_pagebutton" type="button" onclick="location.href='comm_recommend.jsp?pageNum=<%=startPage - 10%>'">다음</button>
<%
 }
 }
 %>

...
<body>
	...
	<div class="comm_head_free">
		<div class="comm_header">
			...
			<div class="comm_header_tap">
				<span class="comm_header_tap_obj_free"><br> <a
					style="color: white" href="comm_recommend.jsp">게임추천</a></span><!--     
         --><span class="comm_header_tap_picked"><br> <a
					href="comm_free.jsp">자유게시판</a></span><!-- 
            --><span class="comm_header_tap_obj_free"><br> <a
					style="color: white" href="comm_news.jsp">게임소식</a></span>
			</div>
		</div>
	</div>
	<div style="width: 67%; margin: auto">
		...
		<form action="comm_free_search.jsp" method="get"
			class="comm_table_search">
			제목 : <input class="comm_table_search_input" type="text" name="text">
		...
		<table class="comm_table">
			...
			%>
			<tr>
				<td style="text-align: left; padding-left: 50px"><a
					href="comm_free_view.jsp?num=<%=num%>"><%=board.getSubject()%></a></td>
				<th class="comm_table_space"></th>
				<td><img src="image/<%=level%>" style="width:25px; height:25px"/> <%=board.getEmail()%>(<%=name%>)</td>
				<th class="comm_table_space"></th>
		...
		<div style="width: 67%; margin: auto; text-align: center">
			...
			%><button class="comm_pagebutton" type="button"
				onclick="location.href='comm_free.jsp?pageNum=<%=startPage - 10%>'">이전</button>
			<%
			}
			...
			%>
			<button class="comm_pagebutton_selected" type="button"
				onclick="location.href='comm_free.jsp?pageNum=<%=i%>'"><%=i%></button>
			<%
			...
			%><button class="comm_pagebutton" type="button"
				onclick="location.href='comm_free.jsp?pageNum=<%=startPage - 10%>'">다음</button>
			<%
			}
			}
			%>
